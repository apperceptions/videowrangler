<?php
/*
 * Created originally as vPIPFeed.php on Apr 25, 2007 by Enric Teller.  Forked as vWranglerFeed.php to 
 * Show In A Box (http://showinabox.tv/) system by Enric Teller in March, 2008.
 * Description: vWranglerFeed Creates feeds for specified media types (i.e., mov, flv, wmv, etc.).
 * Author: Enric Teller
 * Version: 0.22
 ...
 */
	...
	$vsTitleClass = "vwrangler-vs-mediatitle";

	_vWranglerFeed_startup();
	
	function _vWranglerFeed_startup() 
	{
		...
		if ($media == NULL)
			...
			foreach ($aMedia as $media )
			{
				if (substr(trim($media),0,9) == "vs-title:")
				{
					...
					array_push($aSearchPattern, _vWranglerFeed_wildcardToRegex($media));
				...
				$i = count($aExt)-1;
				if (count($aType) < $i+1)
					array_push($aType, _vWranglerFeed_SetType($aExt[$i]));
		...
		if (strpos($blogURL, "?"))
			$feedExt = "&feed=rss2";
		_vWranglerFeed_ffl_HttpGet($blogURL . $feedExt, _vWranglerFeed_processFeed);
	}

	function _vWranglerFeed_processFeed($sContent, $sType)
	{
		global $ChannelTitle, $vsTitle, $aMedia, $aSearchPattern, $aExt, $aType;
		...
		while ($iStartItem !== false)
		{
			...
			$anchorDefault = _vWranglerFeed_GetAnchorDefault($itemArea);
			$itemArea = _vWranglerFeed_striphVlog($itemArea);
			$itemArea = _vWranglerFeed_removeGunk($itemArea, "<![cdata[", "vwrangler_setvisible");
			...
			if ($icontentEncodedStart !== false) 
			{
				...
				while ($iStartA !== false)
				{
					...
					for ($i=0; $i < count($aSearchPattern); $i++)
					{
						...
						$ext = $aExt[$i];
						//If 'vs-title: ', search for "vwrangler-vs-mediatitle" and get media extension location
						if (substr($ext,0,9) == "vs-title:" && strpos(strtolower($anchor), $GLOBALS['vsTitleClass']) !== false)
						{
							...
							if (strtolower($title) == $vsTitle)
							{
								...
								if ($iMediaFound > 0)
								{
									$oExt = _vWranglerFeed_GetExt($anchor);
									$ext = $oExt["ext"];
									$iMediaPos = $oExt["iMediaPos"];
									
									if ($ext !== false)
									{
										$aType[$i] = _vWranglerFeed_SetType($ext);
									}
									else
									{
										$iMediaPos = false;
					...
					else 
					{
						...
						if ($iEndA !== false)
						{
							...
							if ($theExt !== false)
							{
								$hasType = _vWranglerFeed_SetType($theExt);
				...
				while ($iEnclosureStart !== false)
				{
					...
					for ($i=0; $i < count($aSearchPattern); $i++)
					{
						...
						{
							$searchPattern = "/.*" . _vWranglerFeed_regexpEscape($mediaURL) . "/i";
				...
				if ($mediaURL !== false && $mediaURL != null)  
				{
					...
					//if ($fileSize == 0)
					//{
					//	$fileSize = _vWranglerFeed_remote_file_size($mediaURL);
					//}
					...
					{
						$itemArea = _vWranglerFeed_InsertDefault($itemArea, $mediaURL, $anchorDefault);
	...
	function _vWranglerFeed_GetAnchorDefault($content)
	{
		$anchorDefault = false;
	...
	// return false
	function _vWranglerFeed_GetExt($anchor)
	{
	...
	function _vWranglerFeed_findNextEndDIV($content, $iStart)
	{
		//Count DIV's to 0
		$iDIVs = 1;
		$iStartDIV = $iStart;
	...
	function _vWranglerFeed_removeGunk($content, $sStart, $sEnd)
	{
	...
	function _vWranglerFeed_InsertDefault($content, $mediaURL, $anchorDefault)
	{
		$iStarthVlog = strpos(strtolower($content), "hvlog", 0);
	...
	function _vWranglerFeed_SetType($media) {
		$type = false;
		switch ($media) {
			case "mov":
	...
	function _vWranglerFeed_remote_file_size ($url)
	{ 
		$head = ""; 
		$url_p = parse_url($url); 
		$host = $url_p["host"]; 
		...
		if (intval($status)==302 && strlen($newurl) > 0) {
			...
			$return=_vWranglerFeed_remote_file_size($newurl);
		}
		return $return; 
	} 
	
	/**
	 FUNCTION: _vWranglerFeed_ffl_HttpGet()
	 * Perform a HTTP Get Request.
	 *
	 * _vWranglerFeed_ffl_HttpGet uses fsockopen() to request a given URL via HTTP
	 ...
	 * 'errornumber', 'url'.
	 */
	function _vWranglerFeed_ffl_HttpGet( $url, $callBack, $sTermTag = "</item>", $followRedirects=true ) {
	    ...
	    while (!feof($fp)) {
	        ...
	        if ( $foundBody ) {
	            ...
	            if ( ($followRedirects) && (stristr($s, "location:") != false) ) {
	                ...
	                return _vWranglerFeed_ffl_HttpGet( trim($redirect), $callBack, $sTermTag, $followRedirects );
	            }
	            $header .= $s;
	        }
	    }
	...
	*/
	function _vWranglerFeed_wildcardToRegex($wildcard)
	{
	...
	function _vWranglerFeed_regexpEscape($expression)
	{
		$return ="";
